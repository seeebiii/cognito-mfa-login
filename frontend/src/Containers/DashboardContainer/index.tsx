...
class DashBoardContainer extends React.Component<Props, State> {
  ...
  resetPhone = async () => {
    ...
    await Auth.updateUserAttributes(currentAuthenticatedUser, {
      phone_number: '+49170123456789',
    });
  };

  setPhoneCode = async () => {
    await Auth.verifyCurrentUserAttributeSubmit('phone_number', '123456');
  }
        ...
        <Layout>
          ...
          >
            <div className="text-center">
              ...
              <button onClick={() => this.resetPhone()}>Reset Phone</button>
              <button onClick={() => this.setPhoneCode()}>Set phone code</button>
            </div>
          </Layout.Content>
        </Layout>
      </Layout>
